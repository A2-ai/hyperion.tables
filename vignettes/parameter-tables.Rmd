...
call, except using only one line of code. The only intentional difference: 
a single OMEGA section instead of the diagonal/off-diagonal split.
...
If we use the title input but leave it empty, we receive an error message
because title needs an input
...
## Showing how we can get a diagonal/off-diagonal split
