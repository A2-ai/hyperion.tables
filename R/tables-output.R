...
render_gt_parameter_table <- function(layout, spec) {
  ...
  if (ci_in_spec && ci_in_data && isTRUE(spec@ci@merge)) {
    table <- merge_ci_columns(table, pattern = spec@ci@pattern)
  }
  ...
  missing_cols <- missing_apply_columns(layout$params, spec@missing_apply_to)
  if (length(missing_cols) > 0) {
